import logo from "../../logos/burger.png";

const Footer = () => {
    return (
      <>
        <footer>
          <div className="sec1">
            <section className="sec2">
              <img src={logo} alt="" />
              <div className="sec2-emp"></div>
              <section className="selector">
                <div className="India">
                  <img src="data:image/png;base64," alt="" />
                  <p>India</p>
                  <i className="fa-solid fa-chevron-down" style={{ color: '#000000' }}></i>
                </div>
                <div className="English">
                  <i className="fa-solid fa-globe" style={{ color: '#000000' }}></i>
                  <p>English</p>
                  <i className="fa-solid fa-chevron-down" style={{ color: '#000000' }}></i>
                </div>
              </section>
            </section>
            <div className="main-footer">
              <div className="foot1">
                <h6>ABOUT ApniRasoi</h6>
                <nav>
                  <a href="#"><p>Who We Are</p></a>
                  <a href="#"><p>Blog</p></a>
                  <a href="#"><p>Work With Us</p></a>
                  <a href="#"><p>Investor Relations</p></a>
                  <a href="#"><p>Report Fraud</p></a>
                  <a href="#"><p>Contact Us</p></a>
                </nav>
              </div>
              <div className="foot2">
                <h6>ApniRasoiVERSE</h6>
                <nav>
                  <a href="#"><p>ApniRasoi</p></a>
                  <a href="#"><p>Ziakit</p></a>
                  <a href="#"><p>Feeding India</p></a>
                  <a href="#"><p>Hyperpure</p></a>
                  <a href="#"><p>ApniRasoiland</p></a>
                </nav>
              </div>
              <div className="foot3">
                <div className="foot3-1">
                  <h6>FOR RESTAURANTS</h6>
                  <nav>
                    <a href="#"><p>Partner With Us</p></a>
                    <a href="#"><p>Apps For You</p></a>
                  </nav>
                </div>
                <div className="foot3-2">
                  <h6>FOR ENTERPRISES</h6>
                  <nav>
                    <a href="#"><p>ApniRasoi For Enterprise</p></a>
                  </nav>
                </div>
              </div>
              <div className="foot4">
                <h6>LEARN MORE</h6>
                <nav>
                  <a href="#"><p>Privacy</p></a>
                  <a href="#"><p>Security</p></a>
                  <a href="#"><p>Terms</p></a>
                  <a href="#"><p>Sitemap</p></a>
                </nav>
              </div>
              <div className="foot5">
                <h6>SOCIAL LINKS</h6>
                <div className="icons">
                  <a target="_blank" href="https://www.linkedin.com/in/deepak-gupta-97b2641b0/"><i className="fa-brands fa-linkedin" style={{ color: '#000000' }}></i></a>
                  <a target="_blank" href="https://www.linkedin.com/in/deepak-gupta-97b2641b0/"><i className="fa-brands fa-instagram" style={{ color: '#000000' }}></i></a>
                  <a target="_blank" href="https://www.linkedin.com/in/deepak-gupta-97b2641b0/"><i className="fa-brands fa-twitter" style={{ color: '#000000' }}></i></a>
                  <a target="_blank" href="https://www.linkedin.com/in/deepak-gupta-97b2641b0/"><i className="fa-brands fa-youtube" style={{ color: '#000000' }}></i></a>
                  <a target="_blank" href="https://www.linkedin.com/in/deepak-gupta-97b2641b0/"><i className="fa-brands fa-facebook" style={{ color: '#000000' }}></i></a>
                </div>
                <div className="blank1"></div>
                <section className="store">
                  <a href="#">
                    <div className="storeimg">
                      <img src="https://b.zmtcdn.com/data/webuikit/9f0c85a5e33adb783fa0aef667075f9e1556003622.png" alt="App store" />
                    </div>
                  </a>
                  <div className="blank2"></div>
                  <a href="#">
                    <div className="storeimg">
                      <img src="https://b.zmtcdn.com/data/webuikit/23e930757c3df49840c482a8638bf5c31556001144.png" alt="Playstore" />
                    </div>
                  </a>
                </section>
              </div>
            </div>
            <hr className="h-line" />
            <p className="copyright">By continuing past this page, you agree to our Terms of Service, Cookie Policy, Privacy Policy, and Content Policies. All trademarks are properties of their respective owners. 2008-2023 © ApniRasoi Ltd. All rights reserved.</p>
          </div>
        </footer>
      </>
    );
  };
  
  export default Footer;
  